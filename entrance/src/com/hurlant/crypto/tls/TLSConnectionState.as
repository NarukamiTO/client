  ...
  public class TLSConnectionState implements IConnectionState {
    ...
    public function decrypt(param1:uint, param2:uint, param3:ByteArray) : ByteArray {
      ...
      if(this.macAlgorithm != MACs.NULL) {
        ...
        local6 = uint(param3.length - this.hmac.getHashSize());
        local5.writeUnsignedInt(this.seq_hi);
        local5.writeUnsignedInt(this.seq_lo);
        local5.writeByte(param1);
        local5.writeShort(TLSSecurityParameters.PROTOCOL_VERSION);
        local5.writeShort(local6);
        if(local6 != 0) {
          local5.writeBytes(param3,0,local6);
        }
        local7 = this.hmac.compute(this.MAC_write_secret,local5);
        local8 = new ByteArray();
        local8.writeBytes(param3,local6,this.hmac.getHashSize());
    ...
    public function encrypt(param1:uint, param2:ByteArray) : ByteArray {
      ...
      if(this.macAlgorithm != MACs.NULL) {
        ...
        local4.writeShort(TLSSecurityParameters.PROTOCOL_VERSION);
        local4.writeShort(param2.length);
        if(param2.length != 0) {
          local4.writeBytes(param2,0,param2.length);
        }
        local3 = this.hmac.compute(this.MAC_write_secret,local4);
