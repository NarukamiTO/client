package com.hurlant.crypto.symmetric {
  ...
  public class AESKey implements ISymmetricKey {
    ...
    private function expandKey() : void {
      ...
      var local5:uint = 0;
      var local6:uint = 0;
      var local7:uint = this.key.length / 4;
      this.Nr = local7 + 6;
      local6 = local7;
      while(local6 < Nb * (this.Nr + 1)) {
        local1 = uint(this.key[4 * local6 - 4]);
        local2 = uint(this.key[4 * local6 - 3]);
        local3 = uint(this.key[4 * local6 - 2]);
        local4 = uint(this.key[4 * local6 - 1]);
        if(!(local6 % local7)) {
          local5 = local4;
          local4 = uint(Sbox[local1]);
          local1 = uint(Sbox[local2] ^ Rcon[local6 / local7]);
          local2 = uint(Sbox[local3]);
          local3 = uint(Sbox[local5]);
        } else if(local7 > 6 && local6 % local7 == 4) {
          local1 = uint(Sbox[local1]);
          local2 = uint(Sbox[local2]);
          local3 = uint(Sbox[local3]);
          local4 = uint(Sbox[local4]);
        }
        this.key[4 * local6 + 0] = this.key[4 * local6 - 4 * local7 + 0] ^ local1;
        this.key[4 * local6 + 1] = this.key[4 * local6 - 4 * local7 + 1] ^ local2;
        this.key[4 * local6 + 2] = this.key[4 * local6 - 4 * local7 + 2] ^ local3;
        this.key[4 * local6 + 3] = this.key[4 * local6 - 4 * local7 + 3] ^ local4;
        local6++;
      }
    }
    ...
    public function encrypt(param1:ByteArray, param2:uint = 0) : void {
      ...
      local3 = 1;
      while(local3 < this.Nr + 1) {
        if(local3 < this.Nr) {
          this.mixSubColumns();
        } else {
          this.shiftRows();
        }
        this.addRoundKey(this.key,local3 * Nb * 4);
        local3++;
      }
    ...
    public function dispose() : void {
      ...
      while(local1 < this.key.length) {
        this.key[local1] = local2.nextByte();
        local1++;
      }
      this.Nr = local2.nextByte();
      local1 = 0;
      while(local1 < this.state.length) {
        this.state[local1] = local2.nextByte();
        local1++;
      }
      local1 = 0;
      while(local1 < this.tmp.length) {
        this.tmp[local1] = local2.nextByte();
        local1++;
      }
      this.key.length = 0;
      this.keyLength = 0;
    ...
    protected function shiftRows() : void {
      ...
      local1 = uint(Sbox[this.state[1]]);
      this.state[1] = Sbox[this.state[5]];
      this.state[5] = Sbox[this.state[9]];
      this.state[9] = Sbox[this.state[13]];
      this.state[13] = local1;
      local1 = uint(Sbox[this.state[2]]);
      this.state[2] = Sbox[this.state[10]];
      this.state[10] = local1;
      local1 = uint(Sbox[this.state[6]]);
      this.state[6] = Sbox[this.state[14]];
      this.state[14] = local1;
      local1 = uint(Sbox[this.state[15]]);
    ...
    protected function invMixSubColumns() : void {
      ...
      local1 = 0;
      while(local1 < 4 * Nb) {
        this.state[local1] = InvSbox[this.tmp[local1]];
        local1++;
      }
    }
    ...
    protected function addRoundKey(param1:ByteArray, param2:uint) : void {
      ...
      local3 = 0;
      while(local3 < 16) {
        this.state[local3] ^= param1[local3 + param2];
        local3++;
      }
    }
