package com.hurlant.crypto.hash {
  ...
  public class MD2 implements IHash {
    ...
    public function hash(param1:ByteArray) : ByteArray {
      ...
      var local3:uint = uint(uint(16 - param1.length % 16) || 16);
      do {
        param1[param1.length] = local3;
      }
      while(param1.length % 16 != 0);
      var local4:uint = param1.length;
      ...
      while(local3 < local4) {
        local8 = 0;
        while(local8 < 16) {
          local6 = uint(local5[local8] = local5[local8] ^ S[param1[local3 + local8] ^ local6]);
          local8++;
        }
        local3 += 16;
      }
      param1.position = param1.length;
      param1.writeBytes(local5);
      local4 += 16;
      ...
      while(local3 < local4) {
        local8 = 0;
        while(local8 < 16) {
          local7[32 + local8] = (local7[16 + local8] = param1[local3 + local8]) ^ local7[local8];
          local8++;
        }
        local9 = 0;
        local8 = 0;
        while(local8 < 18) {
          local10 = 0;
          while(local10 < 48) {
            local7[local10] = local9 = uint(local7[local10] ^ S[local9]);
            local10++;
          }
          local9 = uint(local9 + local8 & 0xFF);
          local8++;
        }
        local3 += 16;
      }
